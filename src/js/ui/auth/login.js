...
export async function onLogin(event) {
    ...
    if (!emailInput || !passwordInput) return console.error("⚠️ Email or password field missing.");
    ...
    try {
        ...
        if (error.message) {
            errorMessage = error.message; // Direct API message
        } else if (error.errors && error.errors.length > 0 && error.errors[0].message) {
            errorMessage = error.errors[0].message; // If error is in an array
        }

        // **Show Error Modal with Real API Message**
        showActionModal(errorMessage, [
            { text: "Ok", className: "mt-4 bg-royal-blue text-white text-lg font-serif font-bold p-2 rounded w-[100px] h-[30px] flex items-center justify-center" },
        ]);
    } finally {
        spinner.remove();
