...
export async function loadProfileListings() {
  ...
  const profileUsername =
    urlParams.get("user") || localStorage.getItem("username");
  ...
  try {
    const response = await fetch(
      `${API_USER_PROFILE}${profileUsername}/listings?_seller=true&_bids=true`,
      { headers: headers() }
    );
  ...
  try {
    const response = await fetch(
      `${API_USER_PROFILE}${profileUsername}/wins?_seller=true&_bids=true`,
      { headers: headers() }
    );
  ...
  try {
    const response = await fetch(
      `${API_PROFILE_BIDS(profileUsername)}?_listings=true`,
      { headers: headers() }
    );
    ...
    .reduce((uniqueListings, bid) => {
      ...
      if (!existingBid || existingBid.amount < bid.amount) {
        uniqueListings.set(bid.listing.id, {
          ...
          currentHighest: bid.listing.bids?.length
            ? Math.max(...bid.listing.bids.map((b) => b.amount))
            : 0,
          isLeading:
            bid.amount >=
            (bid.listing.bids?.length
              ? Math.max(...bid.listing.bids.map((b) => b.amount))
              : 0),
        });
      }
...
function renderPaginatedListings(items, containerId, paginationId, bid) {
  ...
  if (!container || !paginationContainer) {
    console.error(
      `Pagination container or listing container not found! (ID: ${paginationId})`
    );
    return;
  }
