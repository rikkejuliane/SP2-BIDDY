export function renderProfileListingCard(listing, isLoggedIn) {
  ...
  // ✅ User Info (Ensure avatar and username show)
  ...
  avatar.src = listing.seller?.avatar?.url || "/images/default-avatar.png"; // ✅ Ensure seller data exists
  avatar.alt = `${seller?.name || "Unknown"}'s avatar`;
  avatar.className = "w-10 h-10 rounded-full object-cover";

  const username = document.createElement("span");
  username.className = "font-bold";
  username.textContent = listing.seller?.name || "Unknown";
  ...
  highestBid.textContent = `Highest bid: $${listing.bids?.length > 0 ? Math.max(...listing.bids.map((bid) => bid.amount)) : 0}`;
